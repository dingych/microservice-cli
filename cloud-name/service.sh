...
case "$1" in 
	start)
		...
		then
			...
			exec nohup ${JRE_HOME}/bin/java -Xms128m -Xmx512m -javaagent:/home/yonyou/monitor/pinpoint-bootstrap-1.6.1.jar -Dpinpoint.agentId=admin.222 -Dpinpoint.applicationName=ops-admin  -jar -Deureka.url=${EUREKA_URL} -Dconfig.profile=${CONFIG_PROFILE} ${SERVICE_DIR}/${SERVICE_NAME}\.jar  >/dev/null 2>&1 &
			echo "start success"
		else
